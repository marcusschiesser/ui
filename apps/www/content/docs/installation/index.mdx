...
The installation is analog to the [installation of shadcn/ui](https://ui.shadcn.com/docs/installation/manual) as this project is based on it.
...
To use the components, download the repository as zip archive and extract only the `ui` folder:

```bash
wget https://github.com/marcusschiesser/ui/archive/main.zip
unzip -j main.zip "ui-main/apps/www/registry/default/ui/*" -d ui
```

You can then copy the `ui` folder to the `@components/ui` folder of your app and start using the [components](./docs/components/chat).
